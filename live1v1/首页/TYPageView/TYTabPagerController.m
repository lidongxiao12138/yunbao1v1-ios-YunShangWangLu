...
- (void)configireTabPropertys
{
    ...
    _normalTextFont = [UIFont boldSystemFontOfSize:14];
    _selectedTextFont = [UIFont boldSystemFontOfSize:22];
    _cellSpacing = 0;
    _cellEdging = 0;
    ...
    if (IS_IPHONE_5) {
        _maxWith = _window_width*0.7;
    }else {
        _maxWith = _window_width*0.75;
    }
}
...
- (void)layoutTabPagerView
{
    ...
    switch (self.type) {
        case 0:
            ...
            _collectionViewBar.frame = CGRectMake(0,20+statusbarHeight,_maxWith,44);//_window_width*0.6
            _lineWidth = 15;
            break;
        case 1:
            _pagerBarView.frame = CGRectMake(0,0,CGRectGetWidth(self.view.frame),64+statusbarHeight);
            _collectionViewBar.frame = CGRectMake(_window_width/4,20+statusbarHeight,_window_width/2,44);
            _lineWidth = 30;
            break;
        case 2:
            _pagerBarView.frame = CGRectMake(0,0,CGRectGetWidth(self.view.frame),40);
            _collectionViewBar.frame = CGRectMake(0,0,_window_width,40);
            _lineWidth = 30;
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (animated) {
        [UIView animateWithDuration:_animateDuration animations:^{
            _progressView.frame = CGRectMake(progressX + width/2-_lineWidth/2,progressY+5, _lineWidth, _progressHeight+1);
        }];
    }else {
        _progressView.frame = CGRectMake(progressX + width/2-_lineWidth/2,progressY+5, _lineWidth, _progressHeight+1);
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.type == 0) {
        if (_cellWidth > 0) {
            return CGSizeMake(_maxWith/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
        }else if(_tabDataSourceFlags.titleForIndex){
            return CGSizeMake(_maxWith/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
        }
    }else if (self.type == 1){
        return CGSizeMake(_window_width/4,CGRectGetHeight(_collectionViewBar.frame) - 10);
    }else if (self.type == 2){
        return CGSizeMake(_window_width/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
    }
    return CGSizeZero;
}
// text size
