...
- (void)awakeFromNib {
    ...
    _startRecoedBtn.backgroundColor = normalColors;
...
//        cell.useBtn.layer.borderColor = normalColors.CGColor;
...
- (IBAction)clickUseBtn:(UIButton *)sender {
    ...
    //收藏按钮
//    NSString *url = [purl stringByAppendingFormat:@"?service=Music.collectMusic&uid=%@&token=%@&=%@",[Config getOwnID],[Config getOwnToken],_model.songID];
    [YBToolClass postNetworkWithUrl:@"Music.setCollection" andParameter:@{@"musicid":_model.songID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            
            NSDictionary *infoDic = [info firstObject];
            NSString *isCollect = [NSString stringWithFormat:@"%@",[infoDic valueForKey:@"iscollection"]];
            if ([isCollect isEqual:@"1"]) {
                [_useBtn setImage:[UIImage imageNamed:@"music_collect"] forState:0];
            }else{
                [_useBtn setImage:[UIImage imageNamed:@"music_uncollect"] forState:0];
            }
            [_useBtn.imageView.layer addAnimation:[YBToolClass originToBigToSmallRecovery] forKey:nil];
