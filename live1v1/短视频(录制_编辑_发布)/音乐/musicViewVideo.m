...
//    NSString *url = [purl stringByAppendingFormat:@"?service=Music.hotLists&uid=%@",[Config getOwnID]];
    [YBToolClass postNetworkWithUrl:@"Music.getHot" andParameter:@{@"p":@(_paging)} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
-(void)pullCollectMusic {
    ...
    [YBToolClass postNetworkWithUrl:@"Music.getCollection" andParameter:@{@"p":@(_paging)} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [_tableView.mj_header endRefreshing];
        [_tableView.mj_footer endRefreshing];
        if (code == 0) {
            NSArray *infoA = info;
...
-(void)pullSearchMusic {
    NSString *url = [NSString stringWithFormat:@"Music.search&key=%@&p=%d",self.search.text,_paging];
...
-(UIView *)searchBg {
    if (!_searchBg) {
        ...
        _search.backgroundImage = [YBToolClass getImgWithColor:RGB_COLOR(@"#f9fafb", 1)];
        ...
        [textField setValue:color96 forKeyPath:@"_placeholderLabel.textColor"];
