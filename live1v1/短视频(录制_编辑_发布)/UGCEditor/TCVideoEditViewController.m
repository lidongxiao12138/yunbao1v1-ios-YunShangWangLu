...
- (UIButton *)selVolumeBtn {
    if (!_selVolumeBtn) {
        ...
        [_selVolumeBtn setTitle:@"音量" forState:0];
        ...
        _selVolumeBtn = [YBToolClass setUpImgDownText:_selVolumeBtn];
    }
...
- (UIButton *)selMusicBtn {
    if (!_selMusicBtn) {
        ...
        [_selMusicBtn setTitle:@"音乐" forState:0];
        ...
        _selMusicBtn = [YBToolClass setUpImgDownText:_selMusicBtn];
    }
    return _selMusicBtn;
}
-(UIView *)botBtnMix{
    if (!_botBtnMix) {
        ...
        [_filterfBtn setTitle:@"滤镜" forState:0];
        ...
        _filterfBtn = [YBToolClass setUpImgDownText:_filterfBtn];
        ...
        [_jiancaiBtn setTitle:@"裁剪" forState:0];
        ...
        _jiancaiBtn = [YBToolClass setUpImgDownText:_jiancaiBtn];
        ...
        [_effectBtn setTitle:@"特效" forState:0];
        ...
        _effectBtn = [YBToolClass setUpImgDownText:_effectBtn];
        ...
        _coverBtn = [YBToolClass setUpImgDownText:_coverBtn];
        ...
        _nextBtn.backgroundColor = normalColors;
...
-(void)clickBackBtn {
    UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否确定退出视频编辑" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }];
    [alertContro addAction:cancleAction];
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定退出" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
-(void)clickNextBtn {
    ...
    if (_fileSize > 200 * 1024 * 1024) {
        [MBProgressHUD showError:@"视频文件过大,超过200M！"];
        return;
    }
...
-(instancetype)init {
    ...
    if (self) {
        ...
        _videoOutputPath = [NSTemporaryDirectory() stringByAppendingPathComponent:[YBToolClass getNameBaseCurrentTime:@"outputCut.mp4"]];
        _effectType = -1;
...
- (void)viewDidLoad {
    ...
    _playProgressView.progressTintColor = normalColors;//UIColorFromRGB(0x0accac);
...
- (UIView*)generatingView {
    ...
    if (!_generationView) {
        ...
        _generateProgressView.progressTintColor = normalColors;//UIColorFromRGB(0x0accac);
...
-(void) onGenerateComplete:(TXGenerateResult *)result {
    ...
    if (result.retCode == 0) {
        ...
        UIAlertController * alertC = [UIAlertController alertControllerWithTitle:@"视频生成失败" message:[NSString stringWithFormat:@"错误码：%ld 错误信息：%@",(long)result.retCode,result.descMsg] preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancleA = [UIAlertAction actionWithTitle:@"知道了" style:UIAlertActionStyleCancel handler:nil];
...
- (void)onVideoEnterBackground {
    ...
    if (_generationView && !_generationView.hidden) {
        ...
        UIAlertController * alertC = [UIAlertController alertControllerWithTitle:@"视频生成失败" message:@"中途切后台或则被电话，闹钟等打断导致,请重新生成" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancleA = [UIAlertAction actionWithTitle:@"知道了" style:UIAlertActionStyleCancel handler:nil];
        [alertC addAction:cancleA];
        [self presentViewController:alertC animated:YES completion:nil];
    }
...
- (void)onSetBGMWithFilePath:(NSString *)filePath startTime:(CGFloat)startTime endTime:(CGFloat)endTime {
    if (![_filePath isEqualToString:filePath]) {
        ...
        [_ugcEdit setBGM:filePath result:^(int result) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (result == -1){
                    UIAlertController * alertC = [UIAlertController alertControllerWithTitle:@"设置背景音乐失败" message:@"不支持当前格式的背景音乐!" preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *cancleA = [UIAlertAction actionWithTitle:@"知道了" style:UIAlertActionStyleCancel handler:nil];
                    [alertC addAction:cancleA];
