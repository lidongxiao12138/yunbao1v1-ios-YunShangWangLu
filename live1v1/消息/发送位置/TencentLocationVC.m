...
- (void)viewDidLoad {
    ...
    [self.rightBtn setTitle:@"发送" forState:0];
    _selAddress = @"";
    _selInfo = @"";
...
//    cell.selectedBackgroundView = [[UIImageView alloc]initWithImage:[YBToolClass getImgWithColor:SelCell_Col]];
    return cell;
}
...
-(UIView *)topMix {
    if (!_topMix) {
        ...
        _topMix.backgroundColor = colorf5;
        
        _searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(0,0, _window_width,44)];
        _searchBar.backgroundImage = [YBToolClass getImgWithColor:colorf5];
...
//        [textField setValue:GrayText forKeyPath:@"_placeholderLabel.textColor"];
        ...
        [_cancleBtn setTitleColor:color96 forState:0];
...
- (void)setMapView {
    ...
    UIButton *buttonReset = [[UIButton alloc] initWithFrame:CGRectMake(_window_width - 60, self.mapView.frame.size.height - 65, 50, 50)];
